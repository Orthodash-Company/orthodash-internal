...
async function executeReport(type: ReportType, params: ReportParams): Promise<string> {
  const result = await greyfinchService.makeGraphQLRequest(EXECUTE_REPORT, { type, params } as Record<string, unknown>)
...
async function pollUntilDone(executionId: string): Promise<string> {
  for (let i = 0; i < MAX_POLLS; i++) {
    ...
    const result = await greyfinchService.makeGraphQLRequest(POLL_REPORT, { id: executionId } as Record<string, unknown>)
